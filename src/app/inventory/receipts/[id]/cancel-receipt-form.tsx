...
export function CancelReceiptForm({ receiptId }: Props) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setError(null);
    setPending(true);
    const result = await cancelReceiptAction(receiptId, password, reason);
    setPending(false);
    if (result?.error) {
  ...
  return (
    <form onSubmit={submit} className="rounded-lg border border-slate-700 bg-slate-900/60 p-4">
      ...
      <p className="mt-1 text-xs text-slate-400">
        관리자 비밀번호와 취소 사유를 입력하세요. 재고가 역반영됩니다.
      </p>
      <div className="mt-3 space-y-3">
        <label className="block">
          <span className="mb-1 block text-xs text-slate-400">관리자 비밀번호</span>
          <input
            type="password"
            value={password}
            ...
            placeholder="ADMIN_OVERRIDE_PASSWORD"
          />
        </label>
        <label className="block">
